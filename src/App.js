...
function App() {
    ...
    const [serviceSelected,
        setServiceSelected] = useState({selected: -1, status: false});
    ...
    return (
        <Router>
            <div id="inicio" className="app">
                {!path &&
                <Fragment>
                    <Nav services={services} getSelect={getSelect} select={select} path={path}/>
                    <NavMobile services={services} getSelect={getSelect} select={select} />
                </Fragment>
                } 
                <Routes selected={selected} select={select} getSelect={getSelect} getCopied={getCopied} getPath={getPath}/>
                {!path &&
                <Fragment>
