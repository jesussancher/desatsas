...
class NavMobile extends React.Component {
    ...
    render() {
        ...
        return (
            <Fragment>
            <div id={"burger"} onClick={this.toggleOpen} className={classNames("bars", {'bars-open': open}, {'shadow' : !open})}>
                <img src={open ? x : bars} alt={'Icóno de menú de DESAT'} height="16px" width="16px"/>
            </div>
                <div id={'navboxBack'} onClick={this.closeOnClickOutside} className={'nav-container-back fixed'}>
                    <div id="navbox" className="nav-container absolute shadow">
                        <img src={logo} alt={'Logo de DESAT IPS'} className={'burger-logo'} onClick={() => this.scrollTo("inicio")}/>
                        <nav className="navport">
                            <ul id="navigation">
                                <Link to="/"><li onClick={() => this.scrollTo("inicio")}>
                                    
                                        Inicio
                                </li></Link>
                                <Link to="/"><li  onClick={this.showServices}>
                                    
                                        Servicios
                                </li></Link>
                                <Link to="/"><li onClick={() => this.scrollTo("Contacto")}>
                                    
                                        Contacto
                                </li></Link>
                                <Link to="/Nosotros"><li  onClick={() => this.scrollTo("inicio")}>
